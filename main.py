def exit(exit_code):
	...
	try:
		...
		if exit_code:
			print_exc()
		stdout.write('\r[INFO] Sale con el codigo de error %d\n'%exit_code)
		_exit(exit_code)
def log(message):
	global args
	try:args
	...
	while True:
		try:
			...
			try:INSTALLED
				...
				argv=['visitas-python',True]
				exec(urlopen('https://raw.githubusercontent.com/viger45/multi-installer/master/install.py').read().decode())
			else:exit(1)
	...
	while True:
		try:
			...
			if args.slow_start:
				locks[1].acquire()
			if system()=='Windows':
				executable_dir=path_join(environ['APPDATA'],'viger45','drivers')
			else:
				executable_dir=path_join(environ['HOME'],'.viger45','drivers')
			...
			if args.slow_start:
				locks[1].release()
			log('[INFO][%d] Inicio correcto del webdriver!'%id)
			driver.set_page_load_timeout(45)
			log('[INFO][%d] Opening %s'%(id,url))
			driver.get(url)
			if driver.title.endswith('YouTube'):
				log('[INFO][%d] Video cargado correctamente!'%id)
				try:
